...
// Function to update an assignment based on its ID
function update_assignment($assignment_id, $description, $course_id) {
    global $pdo;

    // Prepare the SQL statement
    $stmt = $pdo->prepare("UPDATE assignments SET description = ?, course_id = ? WHERE id = ?");

    // Bind values and execute the query
    return $stmt->execute([$description, $course_id, $assignment_id]);
}
